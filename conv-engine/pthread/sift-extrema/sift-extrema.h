#ifndef __SIFT_EXTREMA_H__
#define __SIFT_EXTREMA_H__

#include <math.h>
#include <time.h>

#define SIZE 3 												//Fixed Stencil Size  of 3 (both hor and ver)

#define IMAGE_PADDING(NUM)  ( NUM + SIZE - 1)  
#define ZERO_PADDING ( (SIZE - 1)/2)
...
void getPaddedRowCol(image* Image, int dir){
	
			Image->pad_coly = IMAGE_PADDING(Image->coly); 				// IMAGE COLS need to be multiple of kernel size 
			Image->pad_rowx = IMAGE_PADDING(Image->rowx); 			// IMAGE ROWS need to be multiple of kernel size 
}
...
void genImage(image* Image){
	...
	for(i = 0; i < Image->pad_rowx; i++){
		...
		for(j = 0; j < Image->pad_coly; j++){
			...
			if( (j < ZERO_PADDING) || (j > Image->coly) || 
					(i < ZERO_PADDING) || (i > Image->rowx) ){
