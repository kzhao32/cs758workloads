	...
	for(i = 0; i < IMAGE_PADDING(rowx); i=i+SIZE){
		...
		for(j = 0; j < IMAGE_PADDING(coly); j=j+SIZE){
			...
			//for (k = 0; k < SIZE; k++){
				...
				index = INDEX_2D(i, j, rowx, coly, SIZE); 
				...
				*(diffImage + index + 3) = abs(pix1 - pix2);		//diff
				...
				//16
				pix1 = *(inputImage + index + 16);
				pix2 = *(refImage + index + 16);
				*(diffImage + index + 16) = abs(pix1 - pix2);		//diff
				block_sad += abs(pix1 - pix2);							//temp sum

				//17
				pix1 = *(inputImage + index + 17);
				pix2 = *(refImage + index + 17);
				*(diffImage + index + 17) = abs(pix1 - pix2);		//diff
				block_sad += abs(pix1 - pix2);							//temp sum
		
				//18
				pix1 = *(inputImage + index + 18);
				pix2 = *(refImage + index + 18);
				*(diffImage + index + 18) = abs(pix1 - pix2);		//diff
				block_sad += abs(pix1 - pix2);							//temp sum
		
				//19
				pix1 = *(inputImage + index + 19);
				pix2 = *(refImage + index + 19);
				*(diffImage + index + 19) = abs(pix1 - pix2);		//diff
				block_sad += abs(pix1 - pix2);							//temp sum
			
				//32
				pix1 = *(inputImage + index + 32);
				pix2 = *(refImage + index + 32);
				*(diffImage + index + 32) = abs(pix1 - pix2);		//diff
				block_sad += abs(pix1 - pix2);							//temp sum

				//33
				pix1 = *(inputImage + index + 33);
				pix2 = *(refImage + index + 33);
				*(diffImage + index + 33) = abs(pix1 - pix2);		//diff
				block_sad += abs(pix1 - pix2);							//temp sum
		
				//34
				pix1 = *(inputImage + index + 34);
				pix2 = *(refImage + index + 34);
				*(diffImage + index + 34) = abs(pix1 - pix2);		//diff
				block_sad += abs(pix1 - pix2);							//temp sum
		
				//35
				pix1 = *(inputImage + index + 35);
				pix2 = *(refImage + index + 35);
				*(diffImage + index + 35) = abs(pix1 - pix2);		//diff
				block_sad += abs(pix1 - pix2);							//temp sum
		

				//48
				pix1 = *(inputImage + index + 48);
				pix2 = *(refImage + index + 48);
				*(diffImage + index + 48) = abs(pix1 - pix2);		//diff
				block_sad += abs(pix1 - pix2);							//temp sum

				//49
				pix1 = *(inputImage + index + 49);
				pix2 = *(refImage + index + 49);
				*(diffImage + index + 49) = abs(pix1 - pix2);		//diff
				block_sad += abs(pix1 - pix2);							//temp sum
		
				//50
				pix1 = *(inputImage + index + 50);
				pix2 = *(refImage + index + 50);
				*(diffImage + index + 50) = abs(pix1 - pix2);		//diff
				block_sad += abs(pix1 - pix2);							//temp sum
		
				//51
				pix1 = *(inputImage + index + 51);
				pix2 = *(refImage + index + 51);
				*(diffImage + index + 51) = abs(pix1 - pix2);		//diff
				block_sad += abs(pix1 - pix2);							//temp sum
