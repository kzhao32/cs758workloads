...
void blur_filter_hor(image* inputImage, short int* kernel, image* outImage, int size){
	...
	for(i = 0; i < (inputImage->pad_rowx); i++){      //Row-wise
		...
		//for(j = 0; j < (inputImage->coly); j=j+16){      //Column-wise -- 16 element
			...
			int outIndex = (outImage->coly * i);							//Image index
			int inIndex = (inputImage->pad_coly);
			...
			for (k = 0; k < size; k++){
				...
				for (vec = 0; vec < (inputImage->coly); vec++){
					*(outImage->img_ptr + outIndex + vec) = *(outImage->img_ptr + outIndex + vec) +
						(*(inputImage->img_ptr + inIndex + vec + k)) * kernel[k];
...
int main(int argc, char** argv){
		...
		outImage->pad_rowx = x;
		outImage->pad_coly = y;
		outImage->img_ptr = (short int*)malloc(SIZE_INT * (outImage->rowx) * (outImage->coly));	

		//Generate a Gaussian co-efficient kernel
		short int* coeff_kernel = malloc(SIZE_INT * size);
